...
public class DeCompressDistribute {
    ...
    private int x=0;
    DeCompressDistribute(String in,String out) throws IOException {
        this.in = in;
        this.out = out;
        String m = gettype(in);
        if (!m.equals(".rwq")) {
            SceneWarning sceneWarning = new SceneWarning("您选择的文件不是rwq类型，请重新选择");
        } else {
           ...
           return "Outfolder is not exist";
            ...
            if (length > 100000) {
                ...
                for (int i=0;i<filesnumber;i++) {
                    if (i==0) {
                        distribute(this.input, create, (long) (CurrentLoctation+8 + length - 100000), Locations[0]);
                        continue;
                    }
                    if (i<filesnumber-1) {
