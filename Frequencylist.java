...
public class Frequencylist {
  ...
  public Codetree buildcodetree(){
      ...
      while (codetree.size()>1){
          ...
          codetree.add(new Node_frequency(new InternalNode(least1.getNode(),least2.getNode())
                  ,Math.min(least1.getSymbol(),least2.getSymbol()),
                  least1.getFrquencies()+least2.getFrquencies()));
      }
      Codetree codetree1=new Codetree((InternalNode) codetree.remove().getNode(),frequencytable.length);
      return codetree1;
  }
